...
use crate::{
    ...
    workRunner::{WorkRunnerContext, WorkRunnerHandle, thread::ThreadWorkRunner},
};
...
use crate::workRunner::thread::{TASK_RUNS, WORK_RUNS};
...
impl WorkflowServer {
    pub fn new(config: WorkflowServerConfig, host: Host) -> Self {
        ...
        // Spawn inline workRunners.
        let mut workRunners: Vec<WorkRunnerHandle> = Vec::new();
        for wcfg in &config.workRunners {
            let ctx = WorkRunnerContext {
                host:        host_client.clone(),
                persistence: persistence.clone(),
                workRunner_name: wcfg.name.clone(),
            };
            let handle = ThreadWorkRunner::spawn(wcfg, ctx);
            host_client.log("info", "workflow",
                &format!("spawned workRunner '{}' (kind={})", wcfg.name, wcfg.kind));
            workRunners.push(handle);
        }
...
#[component(
    ...
    name        = "WorkflowServer",
    description = "Manages workflow execution: workRunners, work runs, and task runs."
)]
impl WorkflowServer {
    // ── WorkRun lifecycle ────────────────────────────────────────────────────
    ...
    fn create_work_run(&mut self, req: CreateWorkRunRequest) -> Result<WorkRun> {
        ...
        if req.auto_queue {
            ...
            let workRunner = self.state.pick_workRunner()
                .ok_or_else(|| -> Error { "no workRunners available to queue work run".into() })?;
            let workRunner_name = workRunner.name.clone();
            workRunner.enqueue(id.clone()).map_err(|e| -> Error { e.into() })?;

            self.state.host.log("info", "workflow",
                &format!("work run '{id}' queued to workRunner '{workRunner_name}'"));
        }
    ...
    fn queue_work_run(&mut self, req: QueueWorkRunRequest) -> Result<WorkRun> {
        ...
        let workRunner = self.state.pick_workRunner()
            .ok_or_else(|| -> Error { "no workRunners available".into() })?;
    ...
    // ── WorkRunner management ────────────────────────────────────────────────────

    #[handle("workflow/ListWorkRunners")]
    fn list_workRunners(&mut self, _req: serde_json::Value) -> Result<Vec<WorkRunnerSummary>> {
        Ok(self.state.workRunners.iter().map(|w| WorkRunnerSummary {
            name: w.name.clone(),
            kind: w.kind.clone(),
    ...
    fn recover_work_runs(&mut self, _req: serde_json::Value) -> Result<usize> {
        ...
        for run in &active {
            ...
            if phase == WorkRunPhase::Running || phase == WorkRunPhase::Ready {
                if let Some(workRunner) = self.state.pick_workRunner() {
                    if let Err(e) = workRunner.enqueue(id.clone()) {
