...
use workaholic::{
    domain::{
        Artifact, Cron, Group, Namespace, Task, TaskRunnerProfile, Work, WorkRunnerProfile,
    },
...
pub struct CatalogServer {
    ...
    workRunner_profiles: HashMap<String, WorkRunnerProfile>,
...
impl CatalogServer {
    pub fn new(config: CatalogServerConfig, host: Host) -> Self {
        ...
        let mut server = Self {
            ...
            tasks: HashMap::new(),
            crons: HashMap::new(),
            workRunner_profiles: HashMap::new(),
    ...
    fn load_initial_docs(&mut self, path: &str) {
        ...
        for doc in docs {
            ...
            } else if kind.starts_with("orkester/workRunnerprofile:") || kind.starts_with("orkester/workRunner-profile:") {
                match doc.into_typed::<_, ()>() {
                    Ok(v) => { self.workRunner_profiles.insert(key, v); counts[5] += 1; }
                    Err(e) => self.host.log("warn", "catalog", &format!("bad workRunner-profile doc: {e}")),
                }
        ...
        self.host.log("info", "catalog", &format!(
            "loaded: {} namespaces, {} groups, {} works, {} tasks, {} crons, {} workRunner-profiles, {} taskrunner-profiles",
...
impl CatalogServer {
    ...
    #[handle("catalog/CreateWorkRunnerProfile")]
    fn create_workRunner_profile(&mut self, doc: WorkRunnerProfile) -> Result<WorkRunnerProfile> {
        let key = resource_key(&doc.namespace, &doc.name);
        if self.workRunner_profiles.contains_key(&key) {
            return Err(format!("workRunner-profile '{}' already exists", doc.name).into());
        }
        self.workRunner_profiles.insert(key, doc.clone());
        Ok(doc)
    }

    #[handle("catalog/GetWorkRunnerProfile")]
    fn get_workRunner_profile(&mut self, req: ByNameRequest) -> Result<WorkRunnerProfile> {
        let key = resource_key(&req.namespace, &req.name);
        self.workRunner_profiles
            .get(&key)
            .cloned()
            .ok_or_else(|| format!("workRunner-profile '{}' not found", req.name).into())
    }

    #[handle("catalog/SetWorkRunnerProfile")]
    fn set_workRunner_profile(&mut self, doc: WorkRunnerProfile) -> Result<WorkRunnerProfile> {
        let key = resource_key(&doc.namespace, &doc.name);
        self.workRunner_profiles.insert(key, doc.clone());
        Ok(doc)
    }

    #[handle("catalog/DeleteWorkRunnerProfile")]
    fn delete_workRunner_profile(&mut self, req: ByNameRequest) -> Result<DeletedResponse> {
        let key = resource_key(&req.namespace, &req.name);
        Ok(DeletedResponse { deleted: self.workRunner_profiles.remove(&key).is_some() })
    }

    #[handle("catalog/ListWorkRunnerProfiles")]
    fn list_workRunner_profiles(&mut self, req: ListRequest) -> Result<Vec<WorkRunnerProfile>> {
        Ok(self
            .workRunner_profiles
            .values()
