...
//! ## Design
//!
//! Workers call the host with action `runner/Execute`.  Because the component
...
//! multiple worker threads safe in practice even though the SDK dispatch takes
...
/// Sent from a worker to the `RunnerServer` component via `runner/Execute`.
...
impl RunnerServer {
    ...
    /// times out, or is cancelled.  Workers call this on their dedicated
