...
| File | Purpose |
|------|---------|
| `index.html` | Entry point, CDN scripts (Cytoscape, Chart.js), sidebar HTML shell |
...
| `src/api.js` | All HTTP calls (`listWorkRuns()`, `getWorkRun()`, `listCrons()`, etc.) |
...
**Requirements**:
- Call `searchResources({ namespace: getActiveNamespace(), kind, name })` from `../../api.js`
- Toolbar: search input (`q`), kind filter dropdown (populated from unique kinds in results), empty = all
- Table columns: Kind, Name, Version, Namespace, Description (`metadata.description`), Owner (`metadata.owner`)
- Clicking a row navigates to `#/catalog/detail?name=${name}&kind=${encodeURIComponent(kind)}&ns=${ns}`
- Breadcrumb: Catalog
- Page state: `{ q, kindFilter, sortKey, sortDir, page }` — local variables, not persisted
- On namespace change event (`orkester:namespace-changed`), call `setCleanup` and re-navigate
- Listen for namespace change: `window.addEventListener('orkester:namespace-changed', () => navigate('#/catalog'))`

---

### Task 006 — Catalog document detail page
**File**: `src/pages/catalog/detail.js`  
**Route**: `#/catalog/detail?kind=...&name=...&ns=...`

**Requirements**:
- Call `getResource(name)` from `../../api.js`
- Read `kind`, `name`, `ns` from `query` object passed by the router
- Display:
  - Page title: document `name`  
  - Sub-title: `kind` badge (plain text label, not a status badge)
  - Detail grid for metadata fields: namespace, owner, description, tags (comma-separated)
  - **Spec section**: render `spec` as a formatted JSON code block (`<pre class="code-block">`)
  - **Status section**: render `status` as a formatted JSON code block
- Actions: Edit button (opens modal with a `<textarea>` pre-filled with full YAML/JSON, Save calls `updateResource()`), Delete button (confirm then `deleteResource()`, navigate back)
- Breadcrumb: Catalog → `name`
...
**Requirements**:
- Two sections via sub-nav tabs (or just sections with anchors):
  1. **User Preferences** — load/save a `core/UserSettings:1.0` document via `searchResources({ kind: 'core/UserSettings:1.0', namespace: ns })` / `createResource` / `updateResource`. Display as a simple form with a JSON textarea for the `spec` object. 
  2. **App Configuration** — same pattern for `core/AppConfig:1.0`
- If no document exists yet, show an "Initialize" button that calls `createResource()` with an empty spec
- Breadcrumb: Settings
...
**Requirements**:
- Call `searchResources({ kind: 'core/HelpTopic:1.0', namespace: getActiveNamespace() })`
